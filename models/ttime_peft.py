...
import timm
from backbone.lora import TaskWeightedPEFT_ViT_timm
...
class Learner(BaseLearner):
    ...
    def update_network(self, index=True):
        ...
        #     rank = 4
            ...
            rank = 10
        '''
        model = TaskWeightedPEFT_ViT_timm(
            vit_model=model.eval(),
            use_adapter=self.args.get('use_adapter', True),
            adapter_bottleneck=self.args.get('adapter_bottleneck', 64),
            adapter_dropout=self.args.get('adapter_dropout', 0.1),
            adapter_scale=self.args.get('adapter_scale', 1.0),
            use_prompt=self.args.get('use_prompt', False),
            prompt_length=self.args.get('prompt_length', 5),
            num_tasks=self._cur_task + 1,
            current_task=self._cur_task,
            top_k=self.args.get('top_k', 3),
            freeze_backbone=self.args.get('freeze_backbone', True),
            init_from_previous_task=self.args.get('init_from_previous_task', True),
            use_prefix=self.args.get('use_prefix', False),
            prefix_length=self.args.get('prefix_length', 5),
        )
        
        # Load previously saved PEFT parameters for old tasks
        save_dir = self.args['filepath'] + self.args['prefix'] + '/'
        model.load_peft_parameters(save_dir)
            ...
            with torch.no_grad():
                ...
                if self.use_task_prototypes:
                    ...
                    #features = self._network.forward(inputs)['features']#['logits']
                        ...
                        distances = self.compute_distance_to_prototypes(features=features, prototypes=all_prototypes, precisions=all_precisions, distance_metric=self.args.get('distance','mahalanobis'))
                    ...
                    task_weights = self.get_task_weights(distances=distances,weighting_strat=self.args.get('task_weighting_strat', 'softmax'))#
    ...
    def compute_task_prototype_and_covariance_class_based(self, dataloader, device=None, shrinkage=0.05):
        ...
        class_counts = {}      # c -> int
        shared_stats = None    # RunningDiagCov or RunningFullCov

        with torch.no_grad():
            for _, batch in enumerate(dataloader):  # ignore labels, only task-level
                if self.args['dataset']!='bigearthnet':
                ...
                if shared_stats is None:
                        D = feats.size(1)
                        if self.args.get("diag_cov", False):
                            shared_stats = RunningDiagStats(D, device)
                        else:
                            shared_stats = RunningFullStats(D, device)
                ...
                for c in torch.unique(y):
                    ...
                    if c not in class_means:
                        class_means[c] = RunningMean(D, device)
                        class_counts[c] = 0

                    # update class mean
                    class_means[c].update(feats_c)
                    class_counts[c] += feats_c.size(0)

                    # update shared covariance with centered features
                    #centered = feats_c - class_means[c].mean
                    #shared_stats.update(centered)
                shared_stats.update(feats)

        # finalize class prototypes
        #self.class_prototypes = {
        #    c: rm.mean.clone()
        #    for c, rm in class_means.items()
        #}
        labels = sorted(class_means.keys())

        prototypes = torch.stack(
            [class_means[c].mean for c in labels],
            dim=0
        )  # [C, D]

        labels = torch.tensor(labels, device=prototypes.device)        

        # finalize covariance
        if self.args.get("diag_cov", False):
            _, var = shared_stats.finalize()
            var = shrink_diag(var, shrinkage)
            precision = 1.0 / (var + 1e-8)
        else:
            _, cov = shared_stats.finalize()
            cov = shrink_full(cov, shrinkage)
            precision = torch.linalg.pinv(cov)

        print(labels)
        print(prototypes.shape)

        self.task_prototypes[self._cur_task] = prototypes.to(device)
        self.task_precisions[self._cur_task] = precision.to(device)
    ...
    #return task_stats
        ...
        # # Save
        # self.task_prototypes[self._cur_task] = mean_vec.squeeze().to(device)
        # self.task_covariances[self._cur_task] = precision.to(device)
    ...
    def compute_distance_to_prototypes(self, features, prototypes, precisions, distance_metric='cosine'):
        ...
        if not self.args.get('class_based_prototypes', False):
            ...
            for i in range(self._cur_task+1):
                    ...
                    if distance_metric == 'mahalanobis':
                        dist = mahalanobis_distance(features, self.task_prototypes[i], self.task_precisions[i]).min(dim=0, keepdim=True).values
                        print('class based')
