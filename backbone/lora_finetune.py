...
class _LoRA_qkv_timm_train(nn.Module):
    def __init__(self, qkv, linear_a_q, linear_b_q, linear_a_v, linear_b_v, #linear_a_q1, linear_b_q1, linear_a_v1, linear_b_v1,
        task_id, saved_A, saved_B, t_layer_i, rank, scaling_factor, scaling_factor_prev, eval1=False, use_lora=True):
        super().__init__()
...
class _LoRA_qkv_timm_eval(nn.Module):
    ...
    q, k, v = qkv.unbind(0)
    """
    def __init__(self, task_id, qkv: nn.Module, saved_A, saved_B, t_layer_i, rank, scaling_factor,  scaling_factor_prev, save_file, parent_vit, use_lora=True):
        super().__init__()
        self.task_id = task_id
        self.qkv = qkv
        self.dim = qkv.in_features
    ...
    def forward(self, x):
        ...
        if self.use_lora:
            ...
            self.w_a_linear_q.weight.data =  A_q.weight.data# (self.scaling_factor_prev[i]**0.5) * self.parent_vit.tt_weights[i] * A_q.weight.data
            self.w_b_linear_q.weight.data =  B_q.weight.data#(self.scaling_factor_prev[i]**0.5) * self.parent_vit.tt_weights[i] * B_q.weight.data
            self.w_a_linear_v.weight.data =  A_v.weight.data#(self.scaling_factor_prev[i]**0.5) * self.parent_vit.tt_weights[i] * A_v.weight.data
            self.w_b_linear_v.weight.data =  B_v.weight.data#(self.scaling_factor_prev[i]**0.5) * self.parent_vit.tt_weights[i] * B_v.weight.data
...
class LoRA_ViT_timm(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, num_classes: int = 0, increment=10, filepath = './', lora_layer=None, eval=False, index=True, cur_task_index=None, reinit =False, use_lora=True, update_backbone =False):
        ...
        self.lora_alpha = 1.0
        self.multi_rank = [4,4,4,4,4,4,4,4,4,4,4,4]#[10,10,10,10,10,10,10,10,10,10,10,10]#[20,20,20,20,20,20,20,20,20,20,20,20]#[5,5,5,5,10,10,10,10,15,15,15,15]
        self.reinit = reinit
        ...
        for t_layer_i, blk in enumerate(vit_model.blocks):
            ...
            if not eval:
                blk.attn.qkv = _LoRA_qkv_timm_train(
                    ...
                    self.task_id, saved_lora_A, saved_lora_B, t_layer_i, self.multi_rank[t_layer_i] , self.wrapped_param, self.wrapped_param_prev, eval1=False, use_lora=use_lora
                )
            else:
                blk.attn.qkv = _LoRA_qkv_timm_eval(self.task_id, w_qkv_linear, saved_lora_A, saved_lora_B, t_layer_i, self.multi_rank[t_layer_i], self.wrapped_param, self.wrapped_param_prev, self.save_file, parent_vit=self, use_lora=use_lora) 

        if self.task_id == 0:
            self.reset_parameters()
        if self.task_id > 0 and self.reinit:
            self.reset_parameters()
        self.lora_vit = vit_model
        if not eval:
