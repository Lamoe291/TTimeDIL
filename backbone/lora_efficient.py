...
class _LoRA_qkv_timm_train(nn.Module):
    def __init__(self, qkv, linear_a_q, linear_b_q, linear_a_v, linear_b_v, #linear_a_q1, linear_b_q1, linear_a_v1, linear_b_v1,
        task_id, saved_A, saved_B, t_layer_i, rank, scaling_factor, scaling_factor_prev, parent_vit_tt_weights, parent_vit_eval_mode, eval1=False, top_k=3):
        super().__init__()
        ...
        #self.build_adapter_cache()

    def build_adapter_cache(self):
        """Call once when switching to eval mode."""
        with torch.no_grad():
            self._A_q_stack = torch.stack([
                *[self.saved_A[f'saved_A_{i}'][self.t_layer_i*2].weight
                for i in range(self.task_id)],
                self.linear_a_q.weight.clone()
            ])
            self._B_q_stack = torch.stack([
                *[self.saved_B[f'saved_B_{i}'][self.t_layer_i*2].weight
                for i in range(self.task_id)],
                self.linear_b_q.weight.clone()
            ])
            self._A_v_stack = torch.stack([
                *[self.saved_A[f'saved_A_{i}'][self.t_layer_i*2+1].weight
                for i in range(self.task_id)],
                self.linear_a_v.weight.clone()
            ])
            self._B_v_stack = torch.stack([
                *[self.saved_B[f'saved_B_{i}'][self.t_layer_i*2+1].weight
                for i in range(self.task_id)],
                self.linear_b_v.weight.clone()
            ])
    ...
    def forward(self, x):
        ...
        # --- training: only current task adapter, no weighting ---
            ...
            with torch.no_grad():
                ...
                weights_sum = self.tt_weights[:, :T].sum(dim=1)
                active_mask = weights_sum > 0

                new_q = torch.zeros(B, N, D, device=x.device, dtype=x.dtype)
                new_v = torch.zeros(B, N, D, device=x.device, dtype=x.dtype)

                if active_mask.any():
                    # stack once (should be cached — see below)
                    # A_q_stack = torch.stack([
                    #     *[self.saved_A[f'saved_A_{i}'][self.t_layer_i*2].weight
                    #     for i in range(self.task_id)],
                    #     self.linear_a_q.weight
                    # ])  # (T, rank, D)
                    # B_q_stack = torch.stack([
                    #     *[self.saved_B[f'saved_B_{i}'][self.t_layer_i*2].weight
                    #     for i in range(self.task_id)],
                    #     self.linear_b_q.weight
                    # ])  # (T, D, rank)
                    # A_v_stack = torch.stack([
                    #     *[self.saved_A[f'saved_A_{i}'][self.t_layer_i*2+1].weight
                    #     for i in range(self.task_id)],
                    #     self.linear_a_v.weight
                    # ])  # (T, rank, D)
                    # B_v_stack = torch.stack([
                    #     *[self.saved_B[f'saved_B_{i}'][self.t_layer_i*2+1].weight
                    #     for i in range(self.task_id)],
                    #     self.linear_b_v.weight
                    # ])  # (T, D, rank)

                    x_active = x[active_mask]                                 # (B', N, D)
                    w_active = self.tt_weights[active_mask, :T]               # (B', T)

                    topk_weights, topk_indices = w_active.topk(top_k, dim=1)  # (B', k)
                    topk_weights = topk_weights / topk_weights.sum(dim=1, keepdim=True)

                    # ✅ direct index — O(B' * k * rank * D), no expand, no full materialisation
                    A_q_sel = self._A_q_stack[topk_indices]   # (B', k, rank, D)
                    B_q_sel = self._B_q_stack[topk_indices]   # (B', k, D, rank)
                    A_v_sel = self._A_v_stack[topk_indices]   # (B', k, rank, D)
                    B_v_sel = self._B_v_stack[topk_indices]   # (B', k, D, rank)

                    B2 = x_active.shape[0]
                    x_exp = x_active.unsqueeze(1).expand(B2, top_k, N, D)    # (B', k, N, D)

                    h_q  = torch.einsum('bknd,bkrd->bknr', x_exp, A_q_sel)   # (B', k, N, rank)
                    out_q = torch.einsum('bknr,bkdr->bknd', h_q,  B_q_sel)   # (B', k, N, D)
                    h_v  = torch.einsum('bknd,bkrd->bknr', x_exp, A_v_sel)   # (B', k, N, rank)
                    out_v = torch.einsum('bknr,bkdr->bknd', h_v,  B_v_sel)   # (B', k, N, D)

                    w = topk_weights[:, :, None, None]                        # (B', k, 1, 1)
                    new_q[active_mask] = (w * self.scaling_factor[0](out_q)).sum(dim=1)
                    new_v[active_mask] = (w * self.scaling_factor[0](out_v)).sum(dim=1)
        
        
        # else:
        #     T = self.task_id + 1
        #     B, N, D = x.shape
        #     weights_sum = self.tt_weights[:, :T].sum(dim=1)  # (B,)
        #     all_zero = (weights_sum == 0).all()

        #     if all_zero:
        #         # pure backbone — no LoRA contribution at all
        #         new_q = torch.zeros(B, N, D, device=x.device, dtype=x.dtype)
        #         new_v = torch.zeros(B, N, D, device=x.device, dtype=x.dtype)
        #     else:
        #     # --- eval: top-k weighted combination over all tasks ---
        #         with torch.no_grad():
        #             top_k = min(k_value, T)  # gracefully handle T < top_k

        #             # build stacks — replace with self._A_q_stack etc. if caching
        #             A_q_stack = torch.stack([
        #                 *[self.saved_A[f'saved_A_{i}'][self.t_layer_i*2].weight
        #                 for i in range(self.task_id)],
        #                 self.linear_a_q.weight
        ...
        #             # top-k selection — same indices for q and v since routing is per sample
        #             topk_weights, topk_indices = self.tt_weights[:, :T].topk(top_k, dim=1)
        #             topk_weights = topk_weights / topk_weights.sum(dim=1, keepdim=True)  # (B, k)

        #             # gather selected adapter weights
        #             idx_A = topk_indices[:, :, None, None].expand(B, top_k, self.rank, D)
        #             idx_B = topk_indices[:, :, None, None].expand(B, top_k, D, self.rank)

        #             A_q_sel = A_q_stack.unsqueeze(0).expand(B,-1,-1,-1).gather(1, idx_A)  # (B, k, rank, D)
        #             B_q_sel = B_q_stack.unsqueeze(0).expand(B,-1,-1,-1).gather(1, idx_B)  # (B, k, D, rank)
        #             A_v_sel = A_v_stack.unsqueeze(0).expand(B,-1,-1,-1).gather(1, idx_A)  # (B, k, rank, D) — same idx
        #             B_v_sel = B_v_stack.unsqueeze(0).expand(B,-1,-1,-1).gather(1, idx_B)  # (B, k, D, rank) — same idx

        #             # batched LoRA forward for q and v
        #             x_exp = x.unsqueeze(1).expand(B, top_k, N, D)                   # (B, k, N, D)
        ...
        #             w = topk_weights[:, :, None, None]                               # (B, k, 1, 1)
        #             new_q = (w * self.scaling_factor[0](out_q)).sum(dim=1)           # (B, N, D)
        #             new_v = (w * self.scaling_factor[0](out_v)).sum(dim=1)           # (B, N, D)

        # --- shared for both branches: add residuals into qkv ---
        qkv = self.qkv(x)
        qkv[:, :, :self.dim]  += new_q
        qkv[:, :, -self.dim:] += new_v
        return qkv
...
class LoRA_ViT_timm(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, num_classes: int = 0, increment=10, filepath = './', lora_layer=None, eval=False, index=True, cur_task_index=None, top_k=3):
        ...
        for t_layer_i, blk in enumerate(vit_model.blocks):
            ...
            if not eval:
                blk.attn.qkv = _LoRA_qkv_timm_train(
                    ...
                    self.task_id, saved_lora_A, saved_lora_B, t_layer_i, self.multi_rank[t_layer_i] , self.wrapped_param, self.wrapped_param_prev, parent_vit_tt_weights=self.tt_weights, parent_vit_eval_mode=self.eval_mode, eval1=False, top_k=self.top_k
                )
            else:
    ...
    def reset_internal_tt_weights(self,bz=128, full_weight_task_id=0):
        self.tt_weights = torch.zeros(bz, self.task_id+1)
    ...
    def activate_eval(self):
        ...
        for t_layer_i, blk in enumerate(self.lora_vit.blocks):
            ...
            blk.attn.qkv.build_adapter_cache()
